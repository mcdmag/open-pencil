...
- [x] Add `batch` to the MCP tools table/list with description
- [x] Add a "Batch Operations" section under usage/tools with:
...
- [x] Update `create_shape` docs to mention inline style params

### `CHANGELOG.md`

- [x] Add entry under a new version heading:
  ```
  ### Added
...
- [x] Update `packages/docs/programmable/mcp-server.md` to add `batch` tool documentation with schema, examples, and error handling
- [x] Update `packages/docs/programmable/mcp-server.md` to update `create_shape` docs with inline style params
- [x] Update the localized copies (`de/`, `es/`, `fr/`) if they already document individual tools — if they are auto-generated translations, skip them and note that they need re-translation

### Tool description strings

- [x] Update `create_shape`'s `description` field in `packages/core/src/tools/create.ts` to mention inline style support:
  ```
...
- [x] `bun test` — no regressions (documentation changes don't break tests)
- [x] Grep for "batch" in README.md and CHANGELOG.md to confirm entries exist