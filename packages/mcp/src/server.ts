...
import { resolveRefs } from '@open-pencil/core/tools'
        ...
        try {
          ...
          for (let i = 0; i < operations.length; i++) {
            ...
            if (disabledTools.has(op.tool)) {
              return ok({ results, error: { index: i, tool: op.tool, message: `Tool '${op.tool}' is disabled` } })
            }
            if (op.tool === 'batch') {
              return ok({ results, error: { index: i, tool: op.tool, message: 'Recursive batch calls are not allowed' } })
            }
            const resolvedArgs = resolveRefs(op.args, results)
            const result = await sendToBrowser({ command: 'tool', args: { name: op.tool, args: resolvedArgs } })
            const res = result as { ok?: boolean; result?: unknown; error?: string }
            if (res.ok === false) {
              return ok({ results, error: { index: i, tool: op.tool, message: res.error ?? 'Tool failed' } })
            }
            results.push(res.result as Record<string, unknown>)
          }
          return ok({ results })
        } catch (e) {
          return fail(e)
